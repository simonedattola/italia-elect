...
export class WeightsEngine {
  static VERSION = "1.0.0-dynamic";

  /**
   * Calcola i pesi dinamici per un dato territorio e momento storico.
   */
  async computeWeights(
    ...
    const populatedFactors = this.populateFactors(FACTORS, data, party);
    ...
    // Optional: sum weights to 1 (off by default so scenario magnitudes stay interpretable)
  ...
  async buildContextBundle(
    ...
    const factors: FactorDefinition[] = FACTORS.map((template) => {
      const w = byId.get(template.id);
      return {
        ...template,
        rawValue: w?.rawValue ?? template.rawValue,
        weight: w?.weight ?? 0,
        confidence: template.confidence,
      };
    });

    // Re-attach confidence from a fresh populate
    const data = await this.loadContextData(
      geoCode,
      timestamp,
      partySlug(candidateParty),
    );
    const populated = this.populateFactors(
      FACTORS,
      data,
      partySlug(candidateParty),
    );
    const confById = new Map(populated.map((f) => [f.id, f]));
    for (const f of factors) {
      const p = confById.get(f.id);
      if (p) {
        f.confidence = p.confidence;
        f.trend = p.trend;
        f.trendMagnitude = p.trendMagnitude;
        f.rawValue = p.rawValue;
        const w = byId.get(f.id);
        if (w) f.weight = w.weight;
      }
    }
  ...
  private calculateWeights(
    ...
    scenarioType: string,
    _candidateParty: string,
    _historicalData: Record<string, number>,
  ): WeightedFactors[] {
    ...
    for (const factor of factors) {
      ...
      if (scenarioType === "crisis") {
        ...
        // stability (default) + custom treated like stability for base rules
